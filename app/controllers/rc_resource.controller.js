...
exports.findAllFree = (req, res) => {
    rc_resource
        .find()
        .and([
            { $or: [{ access: 'In resource center' }, { access: 'Both in session and in resource center' }] },
            { $or: [{ accesspermission: 'Visitors only' }, { accesspermission: 'All visitors and registered' }] }
        ])
        .exec(function(err, rc_resource) {
            if (err) {
                if (err.kind === "ObjectId") {
